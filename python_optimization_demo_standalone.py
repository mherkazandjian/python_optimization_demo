...
void laplacian_filter_gcc_opt(double *mat, const int m, const int n, double *retval)
{
    ...
    // Process the image by rows - parallelized with OpenMP
    ...
    for (int r = 1; r < m - 1; r++)
    {
        ...
        for (; c + 8 <= n - 1; c += 8)
        {
            ...
            // Load and add adjacent elements in a streamlined way
            result = _mm512_add_pd(result, _mm512_loadu_pd(&mat[i_top]));
            result = _mm512_add_pd(result, _mm512_loadu_pd(&mat[i_bottom]));
            result = _mm512_add_pd(result, _mm512_loadu_pd(&mat[i - 1]));
            result = _mm512_add_pd(result, _mm512_loadu_pd(&mat[i + 1]));
...
## Using gcc
# %%
!gcc laplacian_filter_gcc_opt.c -std=c99 -shared -fPIC -O3 -o laplacian_filter_gcc_opt.so -mavx512f -ffast-math -funroll-loops -ftree-vectorize -fopenmp
# %%
...
# %%
# dump the data into a binary file so that we can use it in the C code by reading it directly
mat_flat.tofile('mat.bin')
...
gflops_results = {key: flops_total / value / 1e9 for key, value in time_results.items()}

# plot the gflops
pyplot.figure(figsize=(10, 6))
bars = pyplot.bar(gflops_results.keys(), gflops_results.values(), color='green', alpha=0.7)
pyplot.ylabel('GFLOPs')
pyplot.title('GFLOPs Performance')
pyplot.xticks(rotation=90)
pyplot.grid(axis='y', linestyle='--', alpha=0.7)

# Plot max possible GFLOPs for one core (67 GFLOPs)
pyplot.axhline(y=67, color='red', linestyle='--', linewidth=2, label='Max 67 GFLOPs')
pyplot.legend()

for bar in bars:
    height = bar.get_height()
    pyplot.text(bar.get_x() + bar.get_width() / 2, height, f'{height:.2f}',
                ha='center', va='bottom')
